...
def pdf_beta (mu, a, b) :
    ...
    where gamma(a+b)/gamma(a)*gamma(b) = 1/beta(a,b)
     ...
     Arguments
    -----------
    mu : numpy array of possible values of a random variable of the beta probability distribution, for which we want to 
...
def initialize_data (X_num, X_mean, mu_num, a, b):
    ...
    mu_num : the number of values taken at equal intervals between 0 and 1 to model the possibilities of 'mu', the probability
             of getting a head when a coin is tossed.
    a, b : the parameters of the beta distribution, which we take as the prior distribution.

     Returns
    ---------
    X : numpy array of data points (1 or 0) representing a random variable of the Bernoulli probability distribution, constructed
        according to the desired specifications (as explained above).
    mu : numpy array of 'mu_num' values between 0 and 1 modelling the possibilities of 'mu', a random variable representing the probability
        of getting a head when a coin is tossed.
    a, b : the parameters of the beta distribution, which we take as the prior distribution. Returned for the sake of clarity in concept,
          these are also parameters set by the user. 
...
def numerical_method_seq (X, mu, a, b) :
    ...
    mu : numpy array of values between 0 and 1 modelling the possibilities of 'mu', a random variable representing the probability
        of getting a head when a coin is tossed.
    a, b : parameters for the beta probability distribution, which we take as the prior distribution.

     Returns
    ---------
    posteriors : a Python list of numpy arrays, each representing the estimated posterior probability distribution, after the 
                observation of each data point.
    '''
...
def plot_sequential_posteriors (mu, posteriors, means = None, method = 'Numerical'):
    ...
    of each data point, as per the sequential method for estimation.
     ...
     Arguments
    -----------
    mu : the random variable assumed to have a prior probability distrubution as beta, taking values between 0 and 1, for 
        which we have estimated the posterior probability distribution. Occupies the x-axis on the plot.
    posteriors : a Python list of numpy arrays, each representing the estimated posterior probability distribution, after the 
                observation of each data point. Each such array occupies the y-axis for its corresponding plot.
    means : a Python list of mean values of the posterior probability distribution estimated after the observation of each data
            point. Optional parameter. Defaults to None.
    method : specifies the technique used for estimation. Can take values 'Numerical' or 'Analytical'. Optional parameter. Defaults 
            to 'Numerical'.
...
def numerical_method_entire (X, mu, a, b):
    ...
    a, b : parameters for the beta probability distribution, which we take as the prior distribution.
     ...
     Returns
    ---------
    posterior_final : a numpy array representing the estimated posterior probability distribution for the mean, after the observation 
                     of the entire dataset at once.
    '''
...
def plot_final_posterior (mu, posterior_final, mean = None, method = 'Numerical'):
    ...
    mu : the random variable assumed to have a prior probability distrubution as beta, taking values between 0 and 1, for 
        which we have estimated the posterior probability distribution. Occupies the x-axis on the plot.
    posterior_final : a numpy array representing the estimated posterior probability distribution for the mean, after the observation 
                     of the entire dataset at once. Occupies the y-axis for its corresponding plot.
    mean : The mean value of the posterior probability distribution estimated after the observation of all data points at once.
